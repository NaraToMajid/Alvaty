...
/* ============================================
   AI ALVATY - Chat with Gemini AI
   ============================================ */
const AI_API_BASE = 'https://api.siputzx.my.id/api/ai/gemini';
const AI_SYSTEM_PROMPT = 'Kamu adalah AI Alvaty, asisten cerdas dari platform sosial media Altavy. Altavy adalah platform sosial media modern dengan fitur: Feed postingan (foto/video/teks), Reels (video pendek), Stories, Chat antar pengguna, Notifikasi, sistem Followers/Following, Hashtag, Mention, Komentar, Like, dan profil pengguna. Kamu membantu pengguna dengan: tips membuat konten menarik, cara menggunakan fitur Altavy, ide postingan kreatif, saran berinteraksi dengan komunitas, tren konten terkini, dan pertanyaan umum lainnya. Jawab dengan ramah, singkat, dan to the point. Gunakan bahasa Indonesia yang natural. Jangan gunakan markdown berlebihan, cukup teks biasa yang mudah dibaca.';
...
async function sendAIMessage() {
  ...
  messagesEl.innerHTML += `<div class="ai-bubble ai-bubble-bot" id="${typingId}"><div class="ai-typing"><span></span><span></span><span></span></div></div>`;
  messagesEl.scrollTop = messagesEl.scrollHeight;

  try {
    const encodedText = encodeURIComponent(text);
    const encodedSystem = encodeURIComponent(AI_SYSTEM_PROMPT);
    const url = `${AI_API_BASE}?text=${encodedText}&cookie=Alvaty&promptSystem=${encodedSystem}`;
    const res = await fetch(url);
    const json = await res.json();
    const reply = json?.data?.response || 'Maaf, aku tidak bisa menjawab saat ini. Coba lagi ya!';
    ...
    const typingEl = document.getElementById(typingId);
    if (typingEl) typingEl.remove();
    appendAIBubble('Koneksi bermasalah. Coba lagi beberapa saat ya!', 'bot');
  }
...
function appendAIBubble(text, type) {
  ...
  // Clean up text: strip markdown bold/italic/headers, keep line breaks
  const clean = text
    .replace(/\*\*(.*?)\*\*/g, '$1')
    .replace(/\*(.*?)\*/g, '$1')
    .replace(/#{1,6}\s/g, '')
    ...
    .replace(/\n{3,}/g, '\n\n')
    .trim();
  div.innerHTML = `<div class="ai-bubble-inner">${clean.replace(/\n/g, '<br>')}</div>`;
